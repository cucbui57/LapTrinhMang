...
public class MicrophoneConnector implements Connector { // Kết nối, đọc dữ liệu từ micphone
    ...
    private DataLine.Info current_dataLineInfo;
    ...
    private void getDevicesList() { // Lấy danh sách cách thiết bị có thể dùng
        ...
        microphoneList.add(new Pair<>("Deafault App's setting", new AudioFormat(8000.0f, 16, 1, true, true)));
    ...
    private void init_connection() throws LineUnavailableException {
        ...
        current_dataLineInfo = new DataLine.Info(TargetDataLine.class, microphoneList.get(current_microphone_index).getValue()); // Thông tin về kết nối
        current_microphone = (TargetDataLine) AudioSystem.getLine(current_dataLineInfo); // Kết nối vào microphone
//        System.out.println(current_microphone.getFormat());
        ...
        data = new byte[current_microphone.getBufferSize() / 5]; // Khởi tạo mảng byte để lưu dữ liệu được đọc từ microphone trong 1 lần (trong 1 lần gọi hàm read();)
...
//        return true;
    ...
    public void listen() { // Lắng nghe microphone
        ...
        if (!listen_to_microphone.isAlive()) {
    ...
    class ListenMicrophone implements Runnable { // Luồng đọc dữ liệu từ microphone
        ...
        public void run() {
            while (is_listening) {
                if(byteArrayOutputStream.size() >= 32767) byteArrayOutputStream.reset();
    ...
    public DataLine.Info getCurrent_dataLineInfo() {
        return current_dataLineInfo;
    }

    public void setCurrent_dataLineInfo(DataLine.Info current_dataLineInfo) {
        this.current_dataLineInfo = current_dataLineInfo;
    }
