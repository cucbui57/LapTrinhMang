...
public class MicrophoneConnector implements Connector { // Kết nối, đọc dữ liệu từ micphone
    ...
    private TargetDataLine microphone; // Microphone đang được sử dụng
    ...
    private void init_connection() throws LineUnavailableException {
        microphone = AudioSystem.getTargetDataLine(microphoneList.get(current_microphone_index).getValue()); // khởi tạo kết nối đến micphone
        ...
        microphone = (TargetDataLine) AudioSystem.getLine(dataLineInfo); // Kết nối vào microphone
        System.out.println(microphone.getFormat());
        microphone.open(); // Mở microphone lên
        CHUNK_SIZE = microphone.getFormat().getFrameSize() * n; // Thiết lập số byte được đọc trong 1 lần (trong 1 lần gọi hàm read();)
        data = new byte[microphone.getBufferSize()]; // Khởi tạo mảng byte để lưu dữ liệu được đọc từ microphone trong 1 lần (trong 1 lần gọi hàm read();)
        microphone.start();
        is_listening = true;
    }
    ...
    class ListenMicrophone implements Runnable { // Luồng đọc dữ liệu từ microphone
        ...
        public void run() {
            while (is_listening) {
                numBytesRead = microphone.read(data, 0, CHUNK_SIZE); // Đọc dữ liệu có độ dài "CHUNK_SIZE" byte vào mảng byte "data"
